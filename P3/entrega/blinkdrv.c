			  ...
			  size_t len, loff_t *off)
{
	...
	unsigned int num1;
	unsigned int num2;
	const char *f1 = "%u:0x%x";

	if(copy_from_user(kbuf,user_buffer,len)) return -EFAULT;

	memset(messages,0,sizeof messages);
	kbuf[len]="\0";
	char *token;
	while((token = strsep(&tmpBuf,",")) != NULL){
		if(sscanf(token,f1,&num1,&num2) == 2 ) {   
			messages[num1][3] = ((num2>>16) & 0xff);
			messages[num1][4] = ((num2>>8) & 0xff);
			messages[num1][5] = (num2 & 0xff);
		}
	}
	...
	for (i=0;i<NR_LEDS;i++){
		...
		message[1]=0x00;
		message[2]=i; 
		message[3]=messages[i][3];
 		message[4]=messages[i][4];
 		message[5]=messages[i][5];
